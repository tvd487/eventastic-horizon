...
const Feature: React.FC<FeatureProps> = ({ icon, title, description }) => {
  return (
    <div className="p-6 border border-gray-200 rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow">
      <div className="w-12 h-12 rounded-full bg-oceanBlue-light flex items-center justify-center mb-4">
        {icon}
      </div>
...
const Features: React.FC = () => {
  const features = [
    {
      icon: <Calendar className="h-6 w-6 text-oceanBlue" />,
    ...
    {
      icon: <Search className="h-6 w-6 text-oceanBlue" />,
